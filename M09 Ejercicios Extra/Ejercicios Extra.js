...
function numberOfCharacters(string) {
  ...
  const newObject = string.split("").reduce((acc, letter) => {
  ...
  //   letrasOrdenadas.forEach((letra) => {
  //     // for of ( por cada letra en... )
  //     if (!nuevoObjeto[letra]) {
  //       // !algo quiere decir que 'si no se cumple ---> ejecute lo siguiente'
...
function capToFront(string) {
  ...
  for (var i = 0; i < string.length; i++) {
    if (string[i] === string[i].toUpperCase()) upper += string[i];
    else lower += string[i];
  }
  return upper + lower;
}
...
function asAmirror(frase) {
  ...
  const reverseWord = fraseSplit.reduce((acc, word) => {
...
function capicua(numero) {
  ...
  const num1 = numero.toString();
  if (num1 === num1.split("").reverse().join("")) return "Es capicua";
...
function sortArray(arrayOfStrings) {
  ...
  const newOrder = arrayOfStrings.sort((a, b) => a.length - b.length);
  return newOrder;
...
function buscoInterseccion(array1, array2) {
  ...
  const finalResult = array1.reduce((acc, number) => {
    if (array2.includes(number)) acc.push(number);
    return acc;
  }, []);
